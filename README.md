...
| `Frequency` | `month` (default), `quarter`, `year`, `topup`, or `hold` — aliases like `monthly` / `yearly` / `annual` / `keep` / `refill` accepted |
...
- **`hold`** (`Misc / Game Credit / 25 / hold`) holds a flat level: when spending dips the pod below `Amount`, the next cycle refills the gap. Never skimmed by `rebalance`, and no scheduled debit is expected — for prepaid balances and spend-and-refill pods.
...
Misc / Game Credit / 25 / hold                 # hold $25 on hand; refill whatever gets spent
```
